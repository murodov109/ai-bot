...
BAD_WORDS = ["jinsi","jinsiy","seks","sex","porn","xxx","qiziqaman","yotsex","18+","fuck","shit"]

def init_db():
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY,username TEXT,image_limit INTEGER DEFAULT 3,chat_limit INTEGER DEFAULT 100,premium INTEGER DEFAULT 0,premium_until TEXT,last_reset TEXT,join_date TEXT)''')
...
async def start_command(client,message:Message):
    ...
    if user_id==ADMIN_ID:
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("📊 Statistika",callback_data="admin_stats")],[InlineKeyboardButton("➕ Kanal qo'shish",callback_data="add_channel"),InlineKeyboardButton("➖ Kanal o'chirish",callback_data="remove_channel")],[InlineKeyboardButton("💎 Premium berish",callback_data="give_premium")],[InlineKeyboardButton("📢 Reklama yuborish",callback_data="send_ad")]])
